...
class LASTstrawDataset(Dataset):
    ...
    def __init__(
        ...
        super(LASTstrawDataset, self).__init__()
    ...
    def get_data(self, idx):
        ...
        semantic_column = "class"

        if not self.cache:
            # data = torch.load(data_path)
            if data_path.suffix==".xyz":
                delimiter = " "
            else:
                delimiter = ","
            fields = ["//X", "Y", "Z", "R", "G", "B", "nx", "ny", "nz", "class"]
            data = pd.read_csv(data_path, usecols=fields, delimiter=delimiter)
        else:
        ...
        coord = data[["//X", "Y", "Z"]].values/1000
        color = data[["R", "G", "B"]].values ## Based on pre-trained scannet
        normal = data[["nx", "ny", "nz"]].values
        scene_id = data_path.stem
        if semantic_column in data.columns:
            segment =data[semantic_column].astype(int).values.reshape([-1]) -1 
            segment[segment==8]=0 # 1-leaves
            segment[segment==8]=0
            # 1-stems
            segment[segment==4]=2 # berry flower crown
            segment[segment==3]=2
            segment[segment==7]=5 #background other, scanning table
            segment[segment==6]=5
            segment[segment==5]=3
    ...
    #     "background", 5
    #     "other", 6 done
    #     "scanning_table", 7 done
    #     "emergent_leaf" 8 done
    # ]
