...
def get_only_parent_nodes(skeleton_data, max_node_order=1):
    ...
    for x in skeleton_data.index[::-1]:
        ...
        # while not root:
            ...
            if skeleton_data.iloc[x].parentid in parentids and skeleton_data.iloc[x].parentid not in edges_dict.keys() and parentid in parentids:
...
def create_skeleton_gt_data(df_gt):
    ## TODO fix node order 
    skeleton_data = df_gt.loc[~df_gt["x_skeleton"].isna(), ["x_skeleton", "y_skeleton", "z_skeleton",	"vid",	"parentid",	"edgetype"]]
